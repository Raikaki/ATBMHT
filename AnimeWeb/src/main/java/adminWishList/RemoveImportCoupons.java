...
@WebServlet(name = "RemoveImportCoupons", value = "/admin/RemoveImportCoupons")
public class RemoveImportCoupons extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Log log = new Log(Log.DANGER, user.getId(), ipClient, "RemoveHistoryImportCoupon", null, 1);
        JDBiConnector jdBiConnector = new JDBiConnector();
        boolean isSuccess = DAORecharge.deleteImportsHistory(id);
